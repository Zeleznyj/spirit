...
void InfoWidget::updateData()
{
    ...
    this->m_Label_NBasis->setText(QString::fromLatin1("N Basis Atoms: ") + QString::number(Geometry_Get_N_Cell_Atoms(this->state.get())));
    int n_cells[3];
    Geometry_Get_N_Cells(this->state.get(), n_cells);
    QString text_Dims = QString::fromLatin1("Cells: ")
        + QString::number(n_cells[0])
        + QString::fromLatin1(" x ")
        + QString::number(n_cells[1])
        + QString::fromLatin1(" x ")
        + QString::number(n_cells[2]);
    int nth = this->spinWidget->visualisationNCellSteps();
    if (nth == 2)
        text_Dims += QString::fromLatin1("\n (showing every ") + QString::number(nth) + QString::fromLatin1("nd atom)");
    else if (nth == 3)
