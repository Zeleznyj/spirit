...
void Method_Solver<Solver::NCG>::Initialize ()
{
    ...
    this->dir_max  = scalarfield( this->noi, 0 );
...
void Method_Solver<Solver::NCG>::Iteration ()
{
    ...
    for (int img=0; img<this->noi; img++)
    {
        ...
        auto& dir_max       = this->dir_max[img];

        beta = Solver_Kernels::ncg_beta_polak_ribiere(image, force, residual, residual_last, torque);
        dir_max = Solver_Kernels::ncg_dir_max(direction, residual, beta, axis);
